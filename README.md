...
This will:
1. Kill any existing processes on ports 8090 and 5174.
...
- **Port 8090 in use?** — Run `./start.sh` (auto-kills), or `lsof -ti:8090 | xargs kill -9`.
- **Frontend can't connect?** — Verify backend at `http://localhost:8090/api/rules`.
- **Vite startup fails with missing `@rollup/rollup-...`?** — Delete `frontend/node_modules` and rerun `bash ./start.sh`; the script now reinstalls optional dependencies automatically on WSL/Linux.
...
- **端口 8090 被占用？** — 运行 `./start.sh`（自动终止），或 `lsof -ti:8090 | xargs kill -9`。
- **前端无法连接后端？** — 确认后端已启动：`http://localhost:8090/api/rules`。
- **Vite 启动报 `@rollup/rollup-...` 缺失？** — 删除 `frontend/node_modules` 后重新执行 `bash ./start.sh`；脚本会在 WSL/Linux 下自动修复 optional dependencies。
